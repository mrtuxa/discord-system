    ...
    public static void main(String[] args) {
        ...
        verify.addReactionRemoveListener(event -> {
            ...
            api.addServerMemberJoinListener(serverMemberJoinEvent -> {
                Optional<TextChannel> channel = api.getTextChannelById(987700321155448872L);
                channel.ifPresent(textChannel -> textChannel.sendMessage(new EmbedBuilder()
                        .setTitle("Over-Network System")
                        .setAuthor("Over-Network", "https://over-network.eu", "https://raw.githubusercontent.com/mrtuxa/bot-images/main/over-hosting_new.png")
                        .setThumbnail(serverMemberJoinEvent.getUser().getAvatar())
                        .addField("Community Chat", "<#987683158097006622>")
                        .setFooter("by Over-Network")));
            });

            api.addServerJoinListener(serverJoinEvent -> System.out.println("Joined Server: " + serverJoinEvent.getServer().getName()));
            api.addServerLeaveListener(serverLeaveEvent -> System.out.println("Left Server: " + serverLeaveEvent.getServer().getName()));
